...
def main() -> int:
    ...
    parser.add_argument(
        "--csv",
        default=None,
        help="Path to annual CSV (defaults to results/Absen-{year}-{TPE/ZL}.csv)",
    )
    parser.add_argument(
        "--input",
    ...
    default_name = f"Absen-{args.year}-{location_code(args.loc)}.csv"
    csv_path = Path(args.csv) if args.csv else Path("results") / default_name
